...
// as a third parameter when creating SessionKeyRequest.
...
$transaction = new \Academe\SagePay\Psr7\Request\Transaction(
    ...
    [
        ...
        'Apply3DSecure' => \Academe\SagePay\Psr7\Request\Transaction::APPLY_3D_SECURE_DISABLE,
    ]
...
if ($transaction_response->getStatus() == '3DAuth') {
    ...
    // This is where ythe bank will return the user when they are finished there.
...
Guzzle does not support `ServerRequestInterface` at this time, so use this instead:

~~~php
$secure3d_server_request = \Academe\SagePay\Psr7\ServerRequest\Secure3DAcs::fromData($_POST);
if ($secure3d_server_request->isValid()) {
    // Yeah, we got a 3d Secure response. Process it here.
    ...
}
...
Handling the 3D Secure result involve two steps:
...
~~~php
    $request = new \Academe\SagePay\Psr7\Request\Secure3D(
        $endpoint,
        $auth,
    ...
    $secure3d_response = new \Academe\SagePay\Psr7\Response\Secure3D($response);
...
~~~php
    ...
    $response = $client->send($transaction_result->message());
    $transaction_fetch = new \Academe\SagePay\Psr7\Response\Transaction($response);
